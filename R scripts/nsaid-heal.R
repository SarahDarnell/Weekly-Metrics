...
for (i in seq_along(plots)) {
  ggsave(
    filename = sprintf("docs/Plots/%s.png", plots[i]),
    plot = get(plots[i]),
...
image_files <- list.files("docs/Plots", pattern = "*.png", full.names = FALSE)

html_images <- lapply(image_files, function(img) {
  tags$img(src = paste0("Plots/", img), style = "width: 80%; margin-bottom: 30px;")
})
...
html_page <- tags$html(
  ...
  tags$body(
    ...
    tags$p("Auto-updated from R 11.12.2025"),
    html_images
  )
)
